...
export const RegistroLabor = ({
    ...
    // Services
    const { crear_labor } = useLabor({ setIsLoading, setError });
    ...
    useEffect(() => {
        ...
        if (id_empresa) {
            get_by_empresa(parseInt(id_empresa)).then((data) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Select
                label="Concesión"
                placeholder={id_empresa ? "Seleccione concesión" : "Primero seleccione una empresa"}
