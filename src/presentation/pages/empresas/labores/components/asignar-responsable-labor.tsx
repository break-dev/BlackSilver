...
import type { RES_HistorialResponsableLabor } from "../../../../../services/labores/dtos/responses";
...
export const AsignarResponsableLabor = ({
  idLabor,
  idEmpresa,
  nombreLabor,
  onSuccess,
}: AsignarResponsableLaborProps) => {
  // States
  ...
  // Services
  const { asignar_responsable, historial_responsables } = useLabores({
    setError,
  });
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    if (success) {
      notifications.show({
        ...
        message: "Nuevo responsable asignado.",
        color: "green",
      });
      setShowForm(false);
      form.reset();
      ...
      <div className="flex justify-between items-center mb-2">
        <div>
          ...
          <p className="text-zinc-500 text-sm">
            {nombreLabor || "Registro histórico"}
          </p>
        </div>
        <Button
          size="xs"
