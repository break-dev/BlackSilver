...
import type { RES_ResponsableMina } from "../../../../../services/empresas/minas/dtos/responses";
...
export const GestionResponsablesMina = ({ idMina, nombreMina }: GestionResponsablesMinaProps) => {
    // Data State
    ...
    const cargarHistorial = async () => {
        setLoading(true);
        const data = await listarResponsables(idMina);
        if (data) {
            // Sort by start date DESC, then by ID DESC
        ...
        return (
            <div className="animate-fade-in space-y-4">
                ...
                <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-900/40">
                    <h3 className="text-white font-bold mb-4">Nueva Asignación de Responsable</h3>

                    <div className="mb-4 p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-lg">
                        <Text size="xs" className="text-indigo-300 font-medium uppercase tracking-wide">Mina:</Text>
                        <Text size="md" fw={700} className="text-white mt-1">{nombreMina}</Text>
                    </div>
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-2">
                <div>
                    ...
                    <p className="text-zinc-500 text-sm">{nombreMina || "Mina"}</p>
                </div>
                <Button
                    size="xs"
                    variant="light"
            ...
            {loading ? (
                ...
                <div className="space-y-3">
                    {responsables.map((item, idx) => {
                        ...
                        const fullName = `${item.apellidos} ${item.nombres}`;
