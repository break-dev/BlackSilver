import { Badge, Button, Group, NumberInput, Stack, Text, Textarea, Paper } from "@mantine/core";
import { useState } from "react";
import { CalculatorIcon, ExclamationTriangleIcon, CheckCircleIcon } from "@heroicons/react/24/outline";
import { useLote } from "../../../../services/lote/useLote";
import type { RES_Lote } from "../../../../services/lote/dtos/responses";
...
export const AjusteStockModal = ({ lote, onSuccess, onCancel }: AjusteStockModalProps) => {
    const [nuevoStock, setNuevoStock] = useState<number | string>(lote.stock_actual);
    const [nuevoStockBase, setNuevoStockBase] = useState<number | string>(lote.stock_actual_base);
    ...
    const handleGuardar = async () => {
        if (isSame) {
            setErrorLocal("El stock ingresado es idéntico al actual. No se requiere ajuste.");
            return;
        }
    ...
    return (
        <Stack gap="lg" className="p-1">
            ...
            <Paper withBorder p="md" radius="lg" bg="zinc.9/20" className="border-zinc-800">
                <Group justify="space-between">
                    <Text fw={800} size="md" className="text-zinc-100 uppercase tracking-tight">
                        {lote.producto}
                    </Text>
                    <Badge variant="light" color="violet" size="sm" radius="sm" className="font-bold border border-violet-500/20">
                        {lote.codigo_lote}
                    </Badge>
                </Group>
            </Paper>
            ...
            <Paper
                ...
                radius="lg"
                bg="zinc.9/10"
                className={`w-full transition-all duration-300 border-zinc-800 ${!isSame ? (diff > 0 ? 'bg-teal-950/20 border-teal-500/30' : 'bg-red-950/20 border-red-500/30') : ''
                    }`}
            >
                <Group justify="center" gap="xl">
                    <div className={`p-2 rounded-full w-10 h-10 flex items-center justify-center transition-colors ${isSame ? 'bg-zinc-800' : diff > 0 ? 'bg-teal-500/10' : 'bg-red-500/10'
                        }`}>
                        {isSame ? (
                    ...
                    <div className="flex flex-col items-center">
                        <Text size="10px" c="zinc.5" fw={800} className="uppercase tracking-widest leading-none mb-1">Variación</Text>
                        <Group gap="xs" align="baseline">
                            <Text size="lg" fw={800} color={isSame ? "zinc.5" : diff > 0 ? "teal" : "red"} className="leading-none">
            ...
            {isSame && (
                <div className="flex items-center gap-3 p-3 bg-zinc-900/50 border border-zinc-800 rounded-xl">
                    ...
                    <Text size="xs" c="amber.5" fw={600}>
                        Aviso: No has realizado ningún cambio. Ingresa una cantidad diferente para aplicar el ajuste.
                    </Text>
                </div>
            )}

            {errorLocal && (
                <Text color="red" size="xs" fw={700} ta="center" className="italic">{errorLocal}</Text>
            )}

            {/* Footer Buttons */}
            <Group justify="flex-end" mt="md" gap="md" className="pt-6 border-t border-zinc-800">
                <Button variant="subtle" color="zinc" onClick={onCancel} radius="md" className="text-zinc-400 hover:text-white px-8 font-bold">
                    Cancelar
                </Button>
                <Button
                    color="indigo"
