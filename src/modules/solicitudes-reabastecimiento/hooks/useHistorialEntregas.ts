...
import type { RES_SolicitudEntrega, RES_SolicitudEntregaDetalle } from "../../../service/responses/solicitudes-reabastecimiento/solicitud-entrega";
...
export const useHistorialEntregas = (idSolicitud: number) => {
  ...
  const loadHistorial = useCallback(async () => {
    try {
      ...
      const entregasSolicitud: RES_SolicitudEntrega[] = (
        res.data.logistica || []
      ).map((ent) => ({
        ...ent,
      ...
      const entregasPrestamo: RES_SolicitudEntrega[] = (
        res.data.prestamo || []
      ).map((ent: RES_SolicitudEntrega) => ({
        ...ent,
        ...
        detalles: (ent.detalles || []).map(
          (d: RES_SolicitudEntregaDetalle) => ({
            ...
            cantidad_solicitud: Number(
              d.cantidad_prestamo || d.cantidad_base || 0,
            ),
            id_unidad_medida_solicitada:
              d.id_unidad_medida_pr || d.id_unidad_medida_solicitada,
            unidad_medida_solicitud_abv:
              d.unidad_medida_pr_abv || d.unidad_medida_solicitud_abv,
            contenido_por_presentacion_solicitado: Number(
              d.contenido_por_presentacion_pr ||
                d.contenido_por_presentacion_solicitado ||
                1,
            ),
            // Mapeo de estados
            estado_entrega_detalle:
