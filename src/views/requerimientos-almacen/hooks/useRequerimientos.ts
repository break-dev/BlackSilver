...
} from "../../views/requerimientos-almacen/services/responses";
import type { DTO_CrearRequerimiento } from "../../views/requerimientos-almacen/services/requests";
...
export const useRequerimientos = ({ setError }: IUseHook) => {
  ...
  const listar = async (filters?: {
    ...
    setError("");
    try {
      const response = await api.get<IRespuesta<RES_RequerimientoAlmacen[]>>(
        path,
        {
          params: filters,
        },
  ...
  const crear = async (dto: DTO_CrearRequerimiento) => {
    setError("");
    try {
      const response = await api.post<IRespuesta<RES_RequerimientoAlmacen>>(
        path,
        dto,
      );
      const result = response.data;
  ...
  const obtenerDetalle = async (id: number) => {
    ...
    try {
      const response = await api.post<
        IRespuesta<RES_RequerimientoDetalleCompleto>
      >(`${path}/obtener-por-id`, {
        id_requerimiento: id,
      });
      const result = response.data;
  ...
  const obtenerTrazabilidad = async (idDetalle: number) => {
    setError("");
    try {
      const response = await api.post<IRespuesta<RES_TrazabilidadEvento[]>>(
