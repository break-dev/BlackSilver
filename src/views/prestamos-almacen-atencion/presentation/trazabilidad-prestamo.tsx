            ...
            return (
              ...
              >
                <Paper p="md" radius="lg" className="bg-zinc-900/40 border border-zinc-800/50 shadow-sm transition-colors hover:bg-zinc-900/60">
                  <Text size="sm" fw={500} c="zinc.100" className="leading-relaxed">
                  ...
                  <div className="mt-3 pt-3 border-t border-zinc-800/50 flex items-center gap-2">
                    ...
                    <Text size="xs" fw={700} c="zinc.3" className="italic underline underline-offset-4 decoration-indigo-500/20">
...
const getSystemMessage = (status: string) => {
  ...
  if (s.includes("pendiente") || s.includes("esperando")) return "Se ha generado la solicitud de préstamo para este producto.";
  if (s.includes("aprobado") || s.includes("autorizado")) return "El almacén prestamista aprobó el despacho para este producto.";
  if (s.includes("despacho") || s.includes("iniciado") || s.includes("entregando")) return "Se ha iniciado la entrega física de los materiales.";
  if (s.includes("entrega") || s.includes("atendido")) return "Se ha registrado una nueva entrega física de este producto.";
  if (s.includes("completad") || s.includes("finalizado")) return "La atención de este ítem se ha completado satisfactoriamente al 100%.";
  if (s.includes("rechazado")) return "El almacén prestamista rechazó la solicitud para este producto.";
  if (s.includes("devolucion")) return "Se ha procesado una devolución de materiales al almacén de origen.";
  if (s.includes("cerrado") || s.includes("anulado")) return "El ítem ha sido cerrado o anulado por el sistema.";
...
const getStatusStyles = (status: string) => {
  ...
  if (s.includes("aprobado") || s.includes("autorizado")) return { color: "violet", variant: "light" as const };
  if (s.includes("despacho") || s.includes("iniciado")) return { color: "orange", variant: "light" as const };
  if (s.includes("entrega") || s.includes("atendido")) {
    if (s.includes("iniciada")) return { color: "orange", variant: "light" as const };
    return { color: "green", variant: "light" as const };
  }
  if (s.includes("completad") || s.includes("finalizado")) return { color: "cyan", variant: "light" as const };
  ...
  if (s.includes("cerrado") || s.includes("anulado")) return { color: "zinc", variant: "filled" as const };
...
const getStatusIcon = (status: string) => {
  ...
  if (s.includes("aprobado") || s.includes("autorizado")) return <CheckBadgeIcon className="w-4 h-4 text-white" />;
  if (s.includes("despacho") || s.includes("iniciado")) return <TruckIcon className="w-4 h-4 text-white" />;
  if (s.includes("entrega") || s.includes("atendido")) return <ArchiveBoxArrowDownIcon className="w-4 h-4 text-white" />;
  if (s.includes("completad") || s.includes("finalizado")) return <CheckCircleIcon className="w-4 h-4 text-white" />;
  if (s.includes("devolucion")) return <TruckIcon className="w-4 h-4 text-white" />; 
  if (s.includes("rechazado")) return <XCircleIcon className="w-4 h-4 text-white" />;
  if (s.includes("cerrado") || s.includes("anulado")) return <CubeIcon className="w-4 h-4 text-white" />;
