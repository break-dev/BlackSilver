...
export const GestionEmpresas = ({ concesion }: GestionEmpresasProps) => {
  ...
  // Services
  const { listarAsignaciones, asignarEmpresa, desasignarEmpresa } =
    useConcesiones({ setError });
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    const success = await asignarEmpresa({
      ...
      fecha_inicio: values.fecha_inicio
        ? values.fecha_inicio.toISOString().split("T")[0]
        : "",
      fecha_fin: null,
    });
  ...
  const handleDesasignar = async (id_asignacion: number, nombre: string) => {
    if (!confirm(`¿Estás seguro de finalizar la asignación de ${nombre}?`))
      return;
