...
export const MinasService = {
  ...
  getConcesionesSesion: async () => {
    const res = await api.get<IRespuesta<RES_ConcesionItem[]>>(`${BASE_MINAS}/concesiones`);
    return res.data;
  },
  ...
  getLabores: async (id_mina: number) => {
    const res = await api.get<IRespuesta<RES_Labor[]>>(`${BASE_MINAS}/labores`, {
      params: { id_mina },
    });
    return res.data;
  },
  ...
  getTiposLabor: async () => {
    const res = await api.get<IRespuesta<RES_TipoLabor[]>>(`${BASE_MINAS}/labores/tipos`);
    return res.data;
  },

  crearLabor: async (dto: DTO_CrearLabor) => {
    const res = await api.post<IRespuesta<RES_Labor>>(`${BASE_MINAS}/labores`, dto);
    return res.data;
  },

  finalizarLabor: async (dto: { id_labor: number; fecha_cierre: string }) => {
    const res = await api.post<IRespuesta<RES_Labor>>(`${BASE_MINAS}/labores/finalizar`, dto);
    return res.data;
  },
};
