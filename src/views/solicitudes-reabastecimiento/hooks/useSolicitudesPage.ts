...
export const useSolicitudesPage = () => {
  ...
  const [solicitudes, setSolicitudes] = useState<RES_SolicitudReabastecimiento[]>([]);

  // Filtros
  const [mes, setMes] = useState<string>(String(new Date().getMonth() + 1));
  const [yearcito, setYearcito] = useState<string>(String(new Date().getFullYear()));
  const [search, setSearch] = useState("");

  // Detalles y Trazabilidad (UI)
  const [selectedReq, setSelectedReq] = useState<RES_SolicitudReabastecimiento | null>(null);
  const [detalles, setDetalles] = useState<RES_SolicitudDetalle[]>([]);
  const [loadingDetalle, setLoadingDetalle] = useState(false);

  const [selectedDetalle, setSelectedDetalle] = useState<RES_SolicitudDetalle | null>(null);
  ...
  const verDetalles = async (req: RES_SolicitudReabastecimiento) => {
    ...
    try {
      const res = await ReabastecimientoService.obtenerDetalles(req.id_solicitud);
      if (res.success) {
  ...
  const verTrazabilidad = async (detalle: RES_SolicitudDetalle) => {
    ...
    try {
      const res = await ReabastecimientoService.obtenerTrazabilidad(detalle.id_solicitud_detalle);
      if (res.success) {
