import { TextInput, Select, Button, Stack, Group, PasswordInput } from '@mantine/core';
import { UserIcon, ShieldCheckIcon, IdentificationIcon, KeyIcon } from '@heroicons/react/24/outline';
...
export const RegistroCuenta = ({ cuentaEdit, onClose, refresh }: RegistroCuentaProps) => {
    const { form, setForm, loading, handleGuardar, roles, empleadosSinCuenta, isEdit } = 
    ...
    return (
        <Stack gap="md">
            <Select
                ...
                data={isEdit ? [{ value: cuentaEdit.id_empleado.toString(), label: `${cuentaEdit.nombre_empleado} ${cuentaEdit.apellido_empleado}` }] : 
                      empleadosSinCuenta.map(e => ({ value: e.id.toString(), label: `${e.apellido}, ${e.nombre}` }))}
            ...
            <Group justify="flex-end" gap="md" mt="xl">
                <Button variant="subtle" onClick={onClose} disabled={loading} radius="lg" size="sm" className="text-zinc-400 hover:text-white hover:bg-zinc-800/50">
                    Cancelar
                </Button>
                <Button
